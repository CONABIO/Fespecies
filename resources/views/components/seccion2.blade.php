<div x-show="step === 2" x-cloak x-transition:enter="transition ease-out duration-300" class="space-y-8 pt-10">
        ...
        <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
            <div class="flex flex-col" x-data="{ openPais: false, filterPais: '' }">
                <label class="text-[22px] font-bold text-slate-700 tracking-tight flex items-center"
                    style="margin-bottom: 10px">1. Distribución mundial:</label>
                ...
                <div class="relative mb-6">
                    <div @click="openPais = !openPais; if(openPais) $nextTick(() => $refs.inputBusquedaPais.focus())"
                        ...
                        <template x-if="form.paises_seleccionados.length === 0">
                            <span class="text-[15px] text-slate-400 p-2 font-bold tracking-widest">Seleccionar país</span>
                        </template>
                    ...
                    <div x-show="openPais" @click.away="openPais = false"
                        ...
                        <div class="max-h-40 overflow-y-auto">
                            <template
                                x-for="option in paisesOptions.filter(i => i.nombrepais.toLowerCase().includes(filterPais.toLowerCase()))"
                                :key="option.paisId">
                                <div @click="if(!form.paises_seleccionados.includes(option.nombrepais)) {
                                        form.paises_seleccionados.push(option.nombrepais);
                                    }
                                    filterPais = '';
                                    $refs.inputBusquedaPais.focus()"
                ...
                <label class="text-[16px] font-bold text-slate-700 tracking-tight flex items-center mb-2">Información
                    adicional</label>
    ...
    x-data="{
        ...
        get filteredEdos() {
            ...
            <h3 class="text-[22px] font-bold text-slate-700 tracking-tight flex items-center">2. Distribución histórica
                en México:
            </h3>
            <div class="space-y-6">
                <div class="flex flex-col">
                    ...
                    <div class="relative mb-4">
                        <div @click="openEdo = !openEdo; if(openEdo) $nextTick(() => $refs.inputBusquedaEdo.focus())"
                            ...
                            <template x-if="form.estados_seleccionados.length === 0">
                                <span class="text-xs text-slate-400 p-2 font-bold tracking-widest">Seleccionar estado</span>
                            </template>
                            <template x-for="sel in form.estados_seleccionados" :key="sel">
                                <div
                                    class="bg-emerald-600 text-white text-[10px] font-black px-4 py-1.5 rounded-xl flex items-center shadow-md">
                                    <span x-text="sel"></span>
                                    <button type="button"
                                        @click.stop="form.estados_seleccionados = form.estados_seleccionados.filter(i => i !== sel); municipiosOptions = municipiosOptions.filter(m => m.nombreEstado !== sel);"
                                        class="ml-2 hover:text-rose-300 transition-colors">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                                d="M6 18L18 6M6 6l12 12"></path>
                        ...
                        <div x-show="openEdo" @click.away="openEdo = false"
                            ...
                            <div class="max-h-40 overflow-y-auto">
                                <div x-show="filterEdo !== '' && !estadosOptions.some(edo => edo.nombreEstado.toLowerCase().includes(filterEdo.toLowerCase()))"
                                    class="px-5 py-4 text-center text-xs text-slate-400 italic font-medium">
                                    <svg class="w-8 h-8 mx-auto mb-2 text-slate-200" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9.172 9.172a4 4 0 0112.728 0M9.172 9.172a4 4 0 0112.728 0M9 10a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 01-1 1h-2a1 1 0 01-1-1v-2zm-3 2a1 1 0 01-1-1V7a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 01-1 1H6z">
                                        </path>
                                    </svg>
                                ...
                                <template
                                    x-for="edo in estadosOptions.filter(i => i.nombreEstado.toLowerCase().includes(filterEdo.toLowerCase()))"
                                    :key="edo.estadoId">
                                    ...
                                    filterEdo = '';
                                    $refs.inputBusquedaEdo.focus()"
                                        class="px-5 py-3 text-xs font-black text-slate-600 hover:bg-emerald-50 cursor-pointer transition-colors border-b border-slate-50 flex justify-between items-center">
                                        <span x-text="edo.nombreEstado"></span>
                                        <svg x-show="form.estados_seleccionados.includes(edo.nombreEstado)"
                                            class="w-3 h-3 text-emerald-500" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                                d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 0114 0z"
                    ...
                    <label
                        class="text-[16px] font-bold text-slate-700 tracking-tight flex items-center mb-2">Información
                        adicional</label>
                    ...
                    <div class="relative mb-4">
                        <div @click="if(municipiosOptions.length > 0) { openMun = !openMun; if(openMun) $nextTick(() => $refs.inputBusquedaMun.focus()) }"
                            ...
                            <template x-if="form.municipios_seleccionados.length === 0">
                                <span class="text-xs text-slate-400 p-2 font-bold  tracking-widest"
                                    x-text="municipiosOptions.length === 0 ? 'Primero selecciona un estado...' : 'Seleccionar municipio...'"></span>
                            </template>
                            <template x-for="sel in form.municipios_seleccionados" :key="sel">
                                <div
                                    class="bg-sky-600 text-white text-[10px] font-black px-4 py-1.5 rounded-xl flex items-center shadow-md">
                                    <span x-text="sel"></span>
                                    <button type="button"
                                        @click.stop="form.municipios_seleccionados = form.municipios_seleccionados.filter(m => m !== sel)"
                                        class="ml-2 hover:text-rose-300 transition-colors">
                        ...
                        <div x-show="openMun" @click.away="openMun = false"
                            class="absolute z-[90] w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden"
                            x-cloak>
                            <div class="p-2 border-b border-slate-100 bg-slate-50">
                                <input type="text" x-model="filterMun" @click.stop x-ref="inputBusquedaMun"
                                    placeholder="Buscar municipio..."
                                    class="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-400">
                            </div>
                            <div class="max-h-40 overflow-y-auto">
                                <div x-show="filterMun !== '' && !municipiosOptions.some(m => form.estados_seleccionados.includes(m.nombreEstado) && m.nombreMunicipio.toLowerCase().includes(filterMun.toLowerCase()))"
                                    class="px-5 py-4 text-center text-xs text-slate-400 italic font-medium">
                                    <svg class="w-8 h-8 mx-auto mb-2 text-slate-200" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <template x-for="edoNombre in form.estados_seleccionados" :key="edoNombre">
                                    <div
                                        x-show="municipiosOptions.some(m => m.nombreEstado === edoNombre && m.nombreMunicipio.toLowerCase().includes(filterMun.toLowerCase()))">
                                        <div
                                            class="bg-slate-100/80 px-4 py-1 text-[9px] font-black text-indigo-600 border-y border-slate-200 sticky top-0">
                                        ...
                                        <template
                                            ...
                                            <div @click="if(!form.municipios_seleccionados.includes(mun.nombreMunicipio)) {
                                                ...
                                                filterMun = '';
                                                $refs.inputBusquedaMun.focus()"
                                                class="px-5 py-2 text-xs font-bold text-slate-600 hover:bg-indigo-50 cursor-pointer border-b border-slate-50 flex justify-between transition-colors">
                                                <span x-text="mun.nombreMunicipio"></span>
                                                <svg x-show="form.municipios_seleccionados.includes(mun.nombreMunicipio)"
                                                    class="w-3 h-3 text-indigo-500" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
