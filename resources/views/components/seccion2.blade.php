                ...
                <div class="relative" @click.away="open = false">
                    <div @click="open = !open"
                        ...
                        <template x-if="form.paises_seleccionados.length === 0">
                            <span
                                class="text-xs text-slate-400 p-2 font-black uppercase tracking-widest">SELECCIONAR...</span>
                        </template>
                    ...
                    <div x-show="open"
                        ...
                        <div class="max-h-40 overflow-y-auto">
                            <template
                                x-for="option in paisesOptions.filter(i => i.PAIS_SA.toLowerCase().includes(filter.toLowerCase()))"
                                :key="option.id">
                                <div @click="if(!form.paises_seleccionados.includes(option.PAIS_SA)) { form.paises_seleccionados.push(option.PAIS_SA); } open = false; filter = ''"
                                    class="px-5 py-3 text-xs font-black text-slate-600 hover:bg-indigo-50 cursor-pointer transition-colors"
                                    x-text="option.PAIS_SA"></div>
                            </template>
                        </div>
                    </div>
                </div>
                <label class="text-[10px] font-black text-slate-500 uppercase mb-3 ml-1 tracking-widest" style="margin-top: 20px">Información adicional:</label>
                <textarea x-model="" class="w-full rounded-2xl border border-slate-200 p-4 text-sm bg-slate-50 focus:border-indigo-400 outline-none min-h-[100px] text-slate-700 font-bold"></textarea>

                <div x-show="form.paises_seleccionados.includes('MEXICO')" x-transition class="...">
                    ...
                    <div class="space-y-6">
                        <div class="flex flex-col">
                            ...
                            <select x-model="form.dist_historica_estado"
                                ...
                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-bold text-slate-700 outline-none focus:border-indigo-400 transition-all">
                                <option value="">Seleccionar estado...</option>
                                <template x-for="edo in estadosOptions" :key="edo.id">
                                    <option :value="edo.id" x-text="edo.EDO_SA"></option>
                                </template>
                            </select>
                            <label class="text-[10px] font-black text-slate-500 uppercase mb-3 ml-1 tracking-widest" style="margin-top: 20px">Información adicional:</label>
                            <textarea x-model="form.endemismo_info" class="w-full rounded-2xl border border-slate-200 p-4 text-sm bg-slate-50 focus:border-indigo-400 outline-none min-h-[100px] text-slate-700 font-bold"></textarea>
                        </div>
                        ...
                        <div class="flex flex-col">
                            ...
                            <select x-model="form.dist_historica_municipio" :disabled="municipiosOptions.length === 0"
                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm font-bold text-slate-700 outline-none focus:border-indigo-400 transition-all disabled:opacity-50">
                                <option value="">Seleccionar municipio...</option>
                                <template x-for="mun in municipiosOptions" :key="mun.id">
                                    <option :value="mun.id" x-text="mun.MUN_SA"></option>
                                </template>
                            </select>
                            <label class="text-[10px] font-black text-slate-500 uppercase mb-3 ml-1 tracking-widest" style="margin-top: 20px">Información adicional:</label>
                <textarea x-model="form.endemismo_info" class="w-full rounded-2xl border border-slate-200 p-4 text-sm bg-slate-50 focus:border-indigo-400 outline-none min-h-[100px] text-slate-700 font-bold"></textarea>
