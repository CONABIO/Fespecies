<div x-show="step === 3" x-cloak x-transition:enter="transition ease-out duration-300" class="max-w-6xl mx-auto space-y-8">
        ...
        <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-8">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div x-show="form.tipoAmbiente === 'Ambiente terrestre' || form.tipoAmbiente === 'Ambiente terrestre-acuático'" x-transition class="space-y-4" x-data="{ openEco: false, filterEco: '' }">
                    <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2"> a) Ecorregiones terrestres </label>
                    <div class="relative">
                        <div @click="openEco = !openEco; if(openEco) $nextTick(() => $refs.inputBusquedaEco.focus())" class="min-h-[50px] p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 flex flex-wrap gap-2 cursor-pointer hover:border-indigo-300 transition-all shadow-inner">
                            <template x-if="form.ecorregiones_terrestres.length === 0">
                                <span class="text-xs text-slate-400 p-2 font-black uppercase tracking-widest"> SELECCIONAR ECORREGIONES... </span>
                            </template>
                            <template x-for="idSel in form.ecorregiones_terrestres" :key="idSel">
                                <div class="bg-indigo-600 text-white text-[10px] font-black px-4 py-1.5 rounded-xl flex items-center shadow-md">
                                    ...
                                    <button type="button" @click.stop="form.ecorregiones_terrestres = form.ecorregiones_terrestres.filter(i => i != idSel)" class="ml-2 hover:text-rose-300">
                        ...
                        <div x-show="openEco" @click.away="openEco = false" x-cloak class="absolute z-[110] w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
                            <div class="p-2 border-b border-slate-100 bg-slate-50">
                                <input type="text" x-model="filterEco" placeholder="Buscar ecorregión..."  x-ref="inputBusquedaEco"  class="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-400">
                            </div>
                            <div class="max-h-60 overflow-y-auto bg-white">
                                <template  x-for="item in ecorregionOptions.filter(i => i.descripcion.toLowerCase().includes(filterEco.toLowerCase()))"   :key="item.ecorregionId">
                                    <div @click="  let id = item.ecorregionId; if(!form.ecorregiones_terrestres.some(s => s == id)) { form.ecorregiones_terrestres = [...form.ecorregiones_terrestres, id]; } openEco = false; filterEco = '';" class="px-5 py-3 text-xs font-black text-slate-600 hover:bg-indigo-50 cursor-pointer border-b border-slate-50 flex justify-between items-center" :class="form.ecorregiones_terrestres.some(s => s == item.ecorregionId) ? 'bg-indigo-50 text-indigo-700' : ''">
                    ...
                    <div class="relative">
                        <div @click="openEcoM = !openEcoM; if(openEcoM) $nextTick(() => $refs.inputBusquedaEcoM.focus())"
                            ...
                            <template x-if="form.ecorregiones_marinas_ids.length === 0">
                                <span class="text-xs text-slate-400 p-2 font-black uppercase tracking-widest">
                                    SELECCIONAR ECORREGIONES MARINAS...
                                </span>
                            </template>
                            ...
                            <template x-for="idSel in form.ecorregiones_marinas_ids" :key="idSel">
                                <div class="bg-indigo-600 text-white text-[10px] font-black px-4 py-1.5 rounded-xl flex items-center shadow-md">
                                    ...
                                    <button type="button" @click.stop="form.ecorregiones_marinas_ids = form.ecorregiones_marinas_ids.filter(i => i != idSel)" class="ml-2 hover:text-rose-300">
                                        ✕
                                    </button>
                                </div>
                            </template>
                        ...
                        <div x-show="openEcoM" @click.away="openEcoM = false" x-cloak
                            ...
                            <div class="p-2 border-b border-slate-100 bg-slate-50">
                                <input type="text" x-model="filterEcoM" placeholder="Buscar ecorregión marina..." x-ref="inputBusquedaEcoM"
                            ...
                            <div class="max-h-60 overflow-y-auto bg-white">
                                <template x-for="item in marinasOptions.filter(i => i.descn1.toLowerCase().includes(filterEcoM.toLowerCase()))" :key="item.idopcion">
                                    <div @click="
                                            if(!form.ecorregiones_marinas_ids.includes(item.idopcion)) {
                                                form.ecorregiones_marinas_ids = [...form.ecorregiones_marinas_ids, item.idopcion];
                                            }
                                            openEcoM = false; filterEcoM = '';
                                        "
                ...
                <div x-show="form.tipoAmbiente" x-transition class="space-y-4" x-data="{ openEcoS: false, filterEcoS: '' }">
                    <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2"> c) Ecosistemas </label>
                    <div class="relative">
                        <div @click="openEcoS = !openEcoS; if(openEcoS) $nextTick(() => $refs.inputBusquedaEcoS.focus())" class="min-h-[50px] p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 flex flex-wrap gap-2 cursor-pointer hover:border-indigo-300 transition-all shadow-inner">
                            <template x-if="form.ecosistemas.length === 0">
                                <span class="text-xs text-slate-400 p-2 font-black uppercase tracking-widest"> SELECCIONAR ECOSISTEMAS... </span>
                            </template>
                        ...
                        <div x-show="openEcoS" @click.away="openEcoS = false" x-cloak class="absolute z-[110] w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
                            <div class="p-2 border-b border-slate-100 bg-slate-50">
                                <input type="text" x-model="filterEcoS" placeholder="Buscar ecosistema..." x-ref="inputBusquedaEcoS" class="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-400">
                            </div>
                ...
                {{-- <div x-show="form.tipoAmbiente" class="pt-4">
                    <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Información adicional ecorregiones</label>
        ...
        <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-10">
            <div class="space-y-4">
                <div class="flex flex-col" x-data="{get agrupadosVeg() { return this.vegetacionOptions.reduce((acc, obj) => { const key = obj.descripcionVegetacion; if (!acc[key]) acc[key] = []; acc[key].push(obj); return acc; }, {});  }}">
                    ...
                    <div class="relative">
                        <div @click="openVeg = !openVeg; if(openVeg) $nextTick(() => $refs.inputBusquedaVeg.focus())" class="min-h-[50px] p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 flex flex-wrap gap-2 cursor-pointer hover:border-indigo-300 shadow-inner">
                            <template x-if="form.tipo_vegetacion_a.length === 0">
                                <span class="text-xs text-slate-400 p-2 font-black uppercase tracking-widest">SELECCIONAR VEGETACIÓN...</span>
                            </template>
                        ...
                        <div x-show="openVeg" @click.away="openVeg = false" x-cloak  class="absolute z-[150] w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
                            ...
                            <div class="max-h-80 overflow-y-auto bg-white">
                                <template x-for="(items, categoria) in agrupadosVeg" :key="categoria">
                                    <div>
                                        <div class="bg-slate-100 px-4 py-2 text-[10px] font-black text-slate-900 uppercase tracking-widest border-y border-slate-200 sticky top-0" x-text="categoria"></div>
                                        <template  x-for="item in items.filter(i => i.descripcionSubVegetacion.toLowerCase().includes(filterVeg.toLowerCase()) || i.descripcionVegetacion.toLowerCase().includes(filterVeg.toLowerCase()))" :key="item.vegetacionId">
                                            <div @click=" if(!form.tipo_vegetacion_a.some(s => s == item.vegetacionId)) { form.tipo_vegetacion_a = [...form.tipo_vegetacion_a, item.vegetacionId]; }openVeg = false; filterVeg = ''; "  class="px-8 py-3 text-xs font-bold text-slate-600 hover:bg-indigo-50 cursor-pointer border-b border-slate-50 flex justify-between items-center" :class="form.tipo_vegetacion_a.some(s => s == item.vegetacionId) ? 'bg-indigo-50 text-indigo-700' : ''">
                    ...
                    <textarea x-model="form.vegetacion_info_adicional_a" placeholder="Recuadro de información adicional..." class="w-full rounded-xl border-2 border-slate-100 p-3 text-xs bg-slate-50 outline-none focus:border-indigo-400 min-h-[60px] mt-4"></textarea>
                </div>
            </div>
            <div class="space-y-4">
                <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block">b) Mencionar las especies asociadas</label>
                <textarea x-model="form.especies_asociadas_info" placeholder="Recuadro de información adicional..." class="w-full rounded-xl border-2 border-slate-100 p-3 text-xs bg-slate-50 outline-none focus:border-indigo-400 min-h-[80px] shadow-inner transition-all"></textarea>
            </div>
            <div class="space-y-6">
                <div class="space-y-6">
                    <div class="flex flex-col" x-data="{ openAntro: false, filterAntro: '' }">
                        ...
                        <div class="relative">
                            <div @click="openAntro = !openAntro; if(openAntro) $nextTick(() => $refs.inputBusquedaAntro.focus())" class="min-h-[50px] p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 flex flex-wrap gap-2 cursor-pointer hover:border-indigo-300 transition-all shadow-inner">
                                <template x-if="form.habitats_antropicos.length === 0">
                                    <span class="text-xs text-slate-400 p-2 font-black uppercase tracking-widest">SELECCIONAR HÁBITAT...</span>
                                </template>
                                <template x-for="idSel in form.habitats_antropicos" :key="idSel">
                                    <div class="bg-indigo-600 text-white text-[10px] font-black px-4 py-1.5 rounded-xl flex items-center shadow-md">
                                        ...
                                        <button type="button" @click.stop="form.habitats_antropicos = form.habitats_antropicos.filter(i => i != idSel)" class="ml-2 hover:text-rose-300">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path>
                                            </svg>
                                        </button>
                            ...
                            <div x-show="openAntro" @click.away="openAntro = false" x-cloak class="absolute z-[110] w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
                                <div class="p-2 border-b border-slate-100 bg-slate-50">
                                    <input type="text" x-model="filterAntro" placeholder="Buscar hábitat..." x-ref="inputBusquedaAntro" class="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-400">
                                </div>
                                <div class="max-h-60 overflow-y-auto bg-white">
                                    <template
                                        x-for="item in habitatsAntropicosOptions.filter(i => i.descn1.toLowerCase().includes(filterAntro.toLowerCase()))"
                                        :key="item.idopcion">
                                        <div @click="
                                let id = item.idopcion;
                                if(!form.habitats_antropicos.some(s => s == id)) {
                                    form.habitats_antropicos = [...form.habitats_antropicos, id];
                                }
                                openAntro = false; filterAntro = '';
                            "
                                            class="px-5 py-3 text-xs font-black text-slate-600 hover:bg-indigo-50 cursor-pointer border-b border-slate-50 flex justify-between items-center"
                                            :class="form.habitats_antropicos.some(s => s == item.idopcion) ?
                                                'bg-indigo-50 text-indigo-700' : ''">
                                            <span x-text="item.descn1"></span>
                                            <template x-if="form.habitats_antropicos.some(s => s == item.idopcion)">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    <template
                        x-for="(label, key) in {
                    habitatAgropecuario: 'i. ¿Hábitat agropecuario?',
                    zonaUrbana: 'ii. ¿Zonas urbanas?',
                    VegetacionSecundaria: 'iii. ¿Vegetación secundaria?'
                }">
            ...
            <div class="space-y-4">
                <div class="flex flex-col" x-data="{ openVegSec: false, filterVegSec: '' }">
                    <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-4">d) Tipo
                        de vegetación secundaria</label>
                    ...
                    <div class="relative">
                        ...
                        <div x-show="openVegSec" @click.away="openVegSec = false" x-cloak
                            class="absolute z-[110] w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
                            <div class="p-2 border-b border-slate-100 bg-slate-50">
                                <input type="text" x-model="filterVegSec" placeholder="Buscar vegetación..."
                                    x-ref="inputBusquedaVegSec"
                                    class="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-400">
                            </div>
                            <div class="max-h-60 overflow-y-auto bg-white">
                                <template
                                    x-for="item in vegSecundariaOptions.filter(i => i.descn1.toLowerCase().includes(filterVegSec.toLowerCase()))"
                                    :key="item.idopcion">
                                    <div @click="
                                let id = item.idopcion;
                                if(!form.vegetacion_secundaria.some(s => s == id)) {
                                    form.vegetacion_secundaria = [...form.vegetacion_secundaria, id];
                                }
                                openVegSec = false; filterVegSec = '';
                            "
                                        class="px-5 py-3 text-xs font-black text-slate-600 hover:bg-indigo-50 cursor-pointer transition-colors border-b border-slate-50 flex justify-between items-center"
                                        :class="form.vegetacion_secundaria.some(s => s == item.idopcion) ?
                                            'bg-indigo-50 text-indigo-700' : ''">
                                        <span x-text="item.descn1"></span>
                                        <template x-if="form.vegetacion_secundaria.some(s => s == item.idopcion)">
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex-grow">
            ...
            <input type="number" step="0.01" readonly
                :value="form.altitud_prom = calcularPromedio(form.intervaloaltitudinalinicial, form.intervaloaltitudinalfinal)"
                class="w-full px-3 py-2 rounded-lg bg-indigo-50 border-none text-sm font-black text-indigo-600 outline-none">
        </div>
    </div>
    <textarea x-model="form.infoAddintervaloaltitudinal" placeholder="Recuadro de información adicional..."
        class="w-full rounded-xl border-2 border-slate-50 p-3 text-xs bg-slate-50 outline-none min-h-[60px]"></textarea>
            ...
            <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-6">
                <div class="flex flex-col" x-data="{
                    ...
                    <h3 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4">18. Clima</h3>
                    <p class="text-[10px] text-slate-500 font-bold uppercase mb-4 italic">Indicar el tipo de clima
                        (solo si está reportado en la bibliografía):</p>

                    <div class="relative">
                        <div @click="openClima = !openClima; if(openClima) $nextTick(() => $refs.inputBusquedaClima.focus())"
                            class="min-h-[50px] p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 flex flex-wrap gap-2 cursor-pointer hover:border-indigo-300 shadow-inner mb-4">

                            <template x-if="form.clima_tipo.length === 0">
                                <span class="text-xs text-slate-400 p-2 font-black uppercase">SELECCIONAR
                                    CLIMA...</span>
                            </template>
                            ...
                            <template x-for="idSel in form.clima_tipo" :key="idSel">
                                <div
                                    ...
                                    <button type="button"
                                        @click.stop="form.clima_tipo = form.clima_tipo.filter(i => i != idSel)"
                                        class="ml-2 hover:text-rose-300">✕</button>
                                </div>
                            </template>
                        </div>
                        <div x-show="openClima" @click.away="openClima = false" x-cloak
                            class="absolute z-[150] w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
                            <div class="p-2 border-b bg-slate-50">
                                <input type="text" x-model="filterClima" placeholder="Buscar clima..."
                                    x-ref="inputBusquedaClima"class="w-full px-3 py-2 text-xs border rounded-lg focus:ring-2 focus:ring-indigo-400 outline-none">
                            </div>
                            <div class="max-h-80 overflow-y-auto bg-white">
                                <template x-for="(items, categoria) in agrupados" :key="categoria">
                                    <div>
                                        <div class="bg-slate-100 px-4 py-2 text-[10px] font-black text-slate-900 uppercase tracking-widest border-y border-slate-200 sticky top-0"
                                            x-text="categoria"></div>
                                        <template
                                            x-for="item in items.filter(i => i.descn2.toLowerCase().includes(filterClima.toLowerCase()) || i.descn1.toLowerCase().includes(filterClima.toLowerCase()))"
                                            :key="item.idopcion">
                                            <div @click="
                                        if(!form.clima_tipo.some(s => s == item.idopcion)) {
                                            form.clima_tipo = [...form.clima_tipo, item.idopcion];
                                        }
                                        openClima = false; filterClima = '';
                                    "
                                                ...
                                                :class="form.clima_tipo.some(s => s == item.idopcion) ?
                                                    'bg-indigo-50 text-indigo-700' : ''">
                                                <span x-text="item.descn2"></span>
                                                <template x-if="form.clima_tipo.some(s => s == item.idopcion)">
                                                    <svg class="w-4 h-4 text-indigo-500" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="3" d="M5 13l4 4L19 7"></path>
                    ...
                    <textarea x-model="form.clima_info" placeholder="Información adicional sobre el clima..."
                        class="w-full rounded-2xl border-2 border-slate-100 p-4 text-sm bg-slate-50 focus:border-indigo-400 outline-none min-h-[100px] shadow-inner mt-4"></textarea>
                </div>
            </div>
            <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-6">
    ...
    <div class="flex items-center gap-3">
        <div class="relative flex-grow">
            <span class="absolute right-3 top-7 text-[10px] font-bold text-slate-400">°C</span>
            <label class="text-[9px] font-bold text-slate-400 uppercase mb-1 block">de</label>
            <input type="number" x-model="form.temperaturainicial"
                class="w-full px-3 py-2 rounded-lg border-2 border-slate-50 bg-slate-50 text-sm outline-none">
        </div>
        <div class="relative flex-grow">
            <span class="absolute right-3 top-7 text-[10px] font-bold text-slate-400">°C</span>
            <label class="text-[9px] font-bold text-slate-400 uppercase mb-1 block">a</label>
            <input type="number" x-model="form.temperaturafinal"
                @change="validarRango('temperaturainicial', 'temperaturafinal', 'Temperatura')"
                class="w-full px-3 py-2 rounded-lg border-2 border-slate-50 bg-slate-50 text-sm outline-none">
        </div>
        <div class="relative flex-grow">
            <span class="absolute right-3 top-7 text-[10px] font-bold text-slate-400">°C</span>
            <label class="text-[9px] font-bold text-slate-400 uppercase mb-1 block">Promedio</label>
            <input type="number" step="0.01" readonly
                :value="form.temp_prom = calcularPromedio(form.temperaturainicial, form.temperaturafinal)"
                class="w-full px-3 py-2 rounded-lg bg-indigo-50 border-none text-sm font-black text-indigo-600 outline-none text-center">
        </div>
    </div>
    <textarea x-model="form.infoaddtemperatura" placeholder="Recuadro de información adicional..."
        class="w-full rounded-xl border-2 border-slate-50 p-3 text-xs bg-slate-50 outline-none min-h-[60px]"></textarea>
</div>
            <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-6">
    ...
    <div class="flex items-center gap-3">
        <div class="relative flex-grow">
            <span class="absolute right-3 top-7 text-[10px] font-bold text-slate-400">mm</span>
            <label class="text-[9px] font-bold text-slate-400 uppercase mb-1 block">de</label>
            <input type="number" x-model="form.precipitacioninicial" min="0"
                class="w-full px-3 py-2 rounded-lg border-2 border-slate-50 bg-slate-50 text-sm outline-none">
        </div>
        <div class="relative flex-grow">
            <span class="absolute right-3 top-7 text-[10px] font-bold text-slate-400">mm</span>
            <label class="text-[9px] font-bold text-slate-400 uppercase mb-1 block">a</label>
            <input type="number" x-model="form.precipitacionfinal" min="0"
                @change="validarRango('precipitacioninicial', 'precipitacionfinal', 'Precipitación')"
                class="w-full px-3 py-2 rounded-lg border-2 border-slate-50 bg-slate-50 text-sm outline-none">
        </div>
        <div class="relative flex-grow">
            <span class="absolute right-3 top-7 text-[10px] font-bold text-slate-400">mm</span>
            <label class="text-[9px] font-bold text-slate-400 uppercase mb-1 block">Promedio</label>
            <input type="number" step="0.01" readonly
                :value="form.prec_prom = calcularPromedio(form.precipitacioninicial, form.precipitacionfinal)"
                class="w-full px-3 py-2 rounded-lg bg-indigo-50 border-none text-sm font-black text-indigo-600 outline-none text-center">
        </div>
    </div>
    <textarea x-model="form.infoaddprecipitacion" placeholder="Recuadro de información adicional..."
        class="w-full rounded-xl border-2 border-slate-50 p-3 text-xs bg-slate-50 outline-none min-h-[60px]"></textarea>
</div>
            <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-6">
    ...
    <div class="flex items-center gap-3">
        <div class="relative flex-grow">
            <span class="absolute right-3 top-7 text-[10px] font-bold text-slate-400">%</span>
            <label class="text-[9px] font-bold text-slate-400 uppercase mb-1 block">de</label>
            <input type="number" x-model="form.humedadinicial" min="0"
                class="w-full px-3 py-2 rounded-lg border-2 border-slate-50 bg-slate-50 text-sm outline-none">
        </div>
        <div class="relative flex-grow">
            <span class="absolute right-3 top-7 text-[10px] font-bold text-slate-400">%</span>
            <label class="text-[9px] font-bold text-slate-400 uppercase mb-1 block">a</label>
            <input type="number" x-model="form.humedadfinal" min="0"
                @change="validarRango('humedadinicial', 'humedadfinal', 'Humedad')"
                class="w-full px-3 py-2 rounded-lg border-2 border-slate-50 bg-slate-50 text-sm outline-none">
        </div>
        <div class="relative flex-grow">
            <span class="absolute right-3 top-7 text-[10px] font-bold text-slate-400">%</span>
            <label class="text-[9px] font-bold text-slate-400 uppercase mb-1 block">Promedio</label>
            <input type="number" step="0.01" readonly
                :value="form.hum_prom = calcularPromedio(form.humedadinicial, form.humedadfinal)"
                class="w-full px-3 py-2 rounded-lg bg-indigo-50 border-none text-sm font-black text-indigo-600 outline-none text-center">
        </div>
    </div>
    <textarea x-model="form.infoaddhumedad" placeholder="Recuadro de información adicional..."
        class="w-full rounded-xl border-2 border-slate-50 p-3 text-xs bg-slate-50 outline-none min-h-[60px]"></textarea>
</div>
            <div class="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                <div class="flex flex-col" x-data="{ openSuelo: false, filterSuelo: '' }">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-6">22. TIPO DE
                        SUELO</label>

                    <div class="relative">
                        <div @click="openSuelo = !openSuelo; if(openSuelo) $nextTick(() => $refs.inputBusquedaSuelo.focus())"
                            class="min-h-[50px] p-3 rounded-2xl border-2 border-slate-100 bg-slate-50 flex flex-wrap gap-2 cursor-pointer shadow-inner">
                            <template x-if="form.suelo_tipo.length === 0">
                                <span class="text-xs text-slate-400 p-2 font-black uppercase">SELECCIONAR
                                    SUELO...</span>
                            </template>
                            <template x-for="idSel in form.suelo_tipo" :key="idSel">
                                <div
                                    ...
                                    <button type="button"
                                        @click.stop="form.suelo_tipo = form.suelo_tipo.filter(i => i != idSel)"
                                        class="ml-2">✕</button>
                                </div>
                            </template>
                        </div>

                        <div x-show="openSuelo" @click.away="openSuelo = false"
                            class="absolute z-[110] w-full mt-2 bg-white border rounded-2xl shadow-2xl" x-cloak>
                            <div class="p-2 border-b bg-slate-50">
                                <input type="text" x-model="filterSuelo" x-ref="inputBusquedaSuelo"
                                    placeholder="Buscar..." class="w-full px-3 py-2 text-xs rounded-lg outline-none">
                            </div>
                            <div class="max-h-60 overflow-y-auto">
                                <template
                                    x-for="opcion in suelosOptions.filter(i => i.descn1.toLowerCase().includes(filterSuelo.toLowerCase()))"
                                    :key="opcion.idopcion">
                                    <div @click="if(!form.suelo_tipo.some(id => id == opcion.idopcion)) { form.suelo_tipo.push(opcion.idopcion); } openSuelo = false; filterSuelo = ''"
                                        class="px-5 py-3 text-xs font-black text-slate-600 hover:bg-indigo-50 cursor-pointer border-b border-slate-50"
                                        :class="form.suelo_tipo.some(id => id == opcion.idopcion) ?
                                            'bg-indigo-50 text-indigo-700' : ''"
                                        x-text="opcion.descn1">
                                    </div>
                                </template>
                            </div>
                        </div>
                    </div>
                    <textarea x-model="form.suelo_info" placeholder="Escribe información adicional aquí..."
                        class="w-full rounded-2xl border-2 border-slate-50 p-4 text-sm bg-slate-50 focus:border-indigo-400 outline-none min-h-[100px] shadow-inner"></textarea>
                </div>
            </div>
            <div
